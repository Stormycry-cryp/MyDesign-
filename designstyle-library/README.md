...
Active readiness is evaluated through both the L3 Markdown validator and the generated L1/L2/design-system validator. In the current v0.2.12 worktree snapshot, the active library has 97 L3 references, 97 cards, 679 L2 dimension summaries, 97 card-linked component-style JSON files, 97 structured `motion.json` files, 97 design-system packs, and 291 generated Apply Pack files. `validate_references.py` and `validate_progressive_library.py` should both pass for the active library before release.
