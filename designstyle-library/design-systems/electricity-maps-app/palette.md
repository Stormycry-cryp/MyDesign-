# Color System: Electricity Maps App

## Observed Palette
| Color | Role | Source | Screenshot Share |
|---|---|---|---|
| `#ffffff` | background | screenshot pixel sample | 0.5733 |
| `#d0b040` | accent | screenshot pixel sample | 0.03 |
| `#50b060` | supporting color | screenshot pixel sample | 0.0283 |
| `#804020` | supporting color | screenshot pixel sample | 0.0261 |
| `#b06030` | supporting color | screenshot pixel sample | 0.0236 |
| `#70c060` | accent | screenshot pixel sample | 0.023 |
| `#b0d050` | accent | screenshot pixel sample | 0.0212 |
| `#f0e050` | accent | screenshot pixel sample | 0.0153 |
| `#c09040` | accent | screenshot pixel sample | 0.0132 |
| `#d0e0e0` | neutral surface | screenshot pixel sample | 0.0131 |
| `#525252` | foreground | explicit reference or DOM color |  |
| `#fafafa` | background | explicit reference or DOM color |  |
| `#212121` | foreground | explicit reference or DOM color |  |
| `#000000` | foreground | explicit reference or DOM color |  |
| `#1863dc` | accent | explicit reference or DOM color |  |

## Mood Keywords
- color-coded
- operational
- clear

## Evidence
- Screenshot: screenshots/electricity-maps-app-desktop.png
- Sampling: sampled 57600 pixels from 1440x1000 PNG
- Sources: screenshot pixel sample, explicit reference or DOM color

## Missing Evidence
- Automated capture covers one desktop viewport and public DOM/resources only.
- Secondary page evidence is summary-only unless listed above; screenshots remain the primary captured page.
- Some CSS/JS may be bundled, minified, blocked, or dynamically injected after capture.

## Do Not Copy
- Do not copy proprietary brand palettes blindly; adapt roles, contrast, and proportions.
